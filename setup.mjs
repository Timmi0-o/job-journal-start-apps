...
const ensureRepo = ({ name, url }) => {
	...
	if (existsSync(dir)) {
		fail(
			`Папка ${name} уже существует, но это не git-репозиторий. Удалите её или переименуйте.`,
		)
	}
		...
		const tryConnect = () => {
			...
			socket.on('error', () => {
				...
				if (Date.now() - startedAt >= timeoutMs) {
					reject(
						new Error(
							`Бэкенд не ответил на порту ${port} за ${timeoutMs / 1000}с`,
						),
					)
					return
				}
...
const main = async () => {
	...
	console.log(`Папка проектов: ${ROOT}`)
